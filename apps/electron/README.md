...
4. **finalize** — the only job that writes to a remote. Refreshes and validates the root `bun.lock` from the released desktop manifest, commits both version files atomically, and tags `desktop-v<version>` in `tagma-mono/main`; only then does it create the GitHub Release pinned to that tag. The stage is idempotent so a re-run after a partial failure can complete what's missing.
