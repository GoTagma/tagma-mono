...
interface LocalPanelProps {
  ...
  /**
   * Force re-stage and re-import an already-loaded plugin. Used by local
   * plugin authors editing under a `file:` / `link:` spec to pick up source
   * changes without a full uninstall/install cycle.
   */
    ...
    <div className="flex flex-col gap-1.5 items-end">
      ...
      {plugin.installed && plugin.loaded && (
        <ActionButton
          ...
          disabled={disabled}
          title="Re-import from source — pick up code changes without uninstalling"
        />
      )}
      {plugin.installed && (
        <ActionButton
