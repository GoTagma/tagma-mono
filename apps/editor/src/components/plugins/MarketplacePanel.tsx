...
function MarketplaceCard({
  ...
  const disabled = actionState.type === 'loading' || actionsDisabled;
...
/**
 * Strict-greater-than comparison over two semver version strings, including
 * prerelease ordering per semver §11. Returns `true` only when `latest` is
 * parseably newer than `installed`. Anything that fails to parse (git URL,
 * `next` dist-tag, missing value) short-circuits to `false` so we never
 * promote an Upgrade button the backend can't actually honor. Mirrors the
 * server's comparator in editor/server/plugins/install.ts so the UI and the
 * /upgrade-plan endpoint agree on what counts as an upgrade.
 */
function isUpgrade(installed: string | null, latest: string): boolean {
  ...
  const parse = (
    v: string,
  ): { mmp: readonly [number, number, number]; pre: readonly string[] } | null => {
    const m = /^v?(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?(?:\+[0-9A-Za-z.-]+)?$/.exec(
      v.trim().replace(/^[=^~]+/, ''),
    );
    if (!m) return null;
    return {
      mmp: [Number(m[1]), Number(m[2]), Number(m[3])] as const,
      pre: m[4] ? m[4].split('.') : [],
    };
  };
  ...
  if (!a || !b) return false;
  for (let i = 0; i < 3; i++) {
    if (b.mmp[i]! > a.mmp[i]!) return true;
    if (b.mmp[i]! < a.mmp[i]!) return false;
  }
  // Same major.minor.patch — compare prerelease identifiers.
  if (a.pre.length === 0 && b.pre.length === 0) return false;
  if (a.pre.length === 0) return false; // stable installed > any prerelease latest
  if (b.pre.length === 0) return true; // prerelease installed < stable latest
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (aIsNum && bIsNum) {
      const diff = Number(bi) - Number(ai);
      if (diff > 0) return true;
      if (diff < 0) return false;
    } else if (aIsNum) {
      return true; // numeric < alphanumeric → latest > installed
    } else if (bIsNum) {
      return false;
    } else if (ai !== bi) {
      return bi > ai;
