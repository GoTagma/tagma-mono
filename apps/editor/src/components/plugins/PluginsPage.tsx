...
export function PluginsPage({
  ...
  const [upgradeConfirm, setUpgradeConfirm] = useState<Extract<
    PluginUpgradePlan,
    { status: 'ready' }
  > | null>(null);
  ...
  const handleInstall = useCallback(
    async (name: string, version?: string) => {
      ...
      try {
        ...
        // two cases the YAML must NOT pick up:
        //   - rollback path: install failed to load and prior state was
        //     restored, so the package is no longer installed at the version
        //     the user just tried.
        //   - partial path: install succeeded but load failed, so referencing
        ...
        // the same hydration path the manual plugin refresh / workspace-open uses so the Task
    ...
    async (name: string) => {
      ...
      try {
        ...
        finishAction({
          ...
          message:
            result.warning ??
            (result.upgraded && result.upgraded.length > 1
              ? `Upgraded ${result.upgraded.length} plugins`
              : result.plugin.version
                ? `Upgraded to v${result.plugin.version}`
                : 'Upgraded'),
        });
      } catch (e: unknown) {
        const message = extractErrorMessage(e);
  ...
  const handleUninstall = useCallback(
    async (name: string) => {
      ...
      if (impact.impacts.length === 0) {
        await performUninstall(name, impact.impacts);
        return;
      }
  ...
  // local plugin author edits source under a `file:`/`link:` spec and wants
  // the live handler to pick up changes without a full uninstall/install.
  // The server distinguishes this from Load via `force:true`; otherwise the
    ...
    <div className="h-full flex flex-col bg-tagma-bg">
      ...
      {uninstallConfirm && (
        <UninstallConfirmDialog
          ...
          onConfirm={() => {
            ...
            const impacts = uninstallConfirm.impacts;
            setUninstallConfirm(null);
            performUninstall(name, impacts);
          }}
        />
      )}
      ...
      {upgradeConfirm && (
        <UpgradeConfirmDialog
          ...
          onConfirm={() => {
            const name = upgradeConfirm.target;
...
function UpgradeConfirmDialog({
  ...
  plan: Extract<PluginUpgradePlan, { status: 'ready' }>;
  onCancel: () => void;
  onConfirm: () => void;
}) {
  return (
