...
interface MinimapProps {
  ...
  /**
   * Optional folders override. When omitted, reads from the pipeline store
   * (editor mode). Run mode currently does not pass this — the live editor
   * folder layout drives the minimap; an empty array suppresses folder rows
   * entirely.
   */
...
export function Minimap({
  ...
  return (
    <div
      className="minimap-float absolute z-20 bg-tagma-surface/90 border border-tagma-border shadow-panel"
    ...
    >
      ...
      <div className="p-1.5">
        ...
        >
          {(() => {
            // Walk the render plan to draw row stripes that match the canvas
            // exactly — folder bars get a dimmer band, tracks alternate the
            // standard zebra colors. Tracks in a collapsed folder are absent
            // from the plan, so the canvas height matches naturally.
            let trackZebraIdx = 0;
            let yAcc = 0;
            ...
            for (const row of renderPlan) {
              ...
              const fill = isFolder
                ? 'var(--tagma-mm-stripe-a)'
                : trackZebraIdx % 2 === 0
                  ? 'var(--tagma-mm-stripe-a)'
                  : 'var(--tagma-mm-stripe-b)';
              stripes.push(
                <rect
                  key={isFolder ? `mm-folder-${row.folderId}` : `mm-row-${row.trackId}`}
                  x={offsetX}
                  y={offsetY + yAcc * scale}
                  width={contentW * scale}
                  height={rowH * scale}
                  style={{ fill }}
                />,
              );
              if (!isFolder) trackZebraIdx += 1;
              yAcc += rowH;
            }
            return stripes;
          })()}
