...
function AssistantResponseControls({ info, text }: { info: AssistantMessage; text: string }) {
  ...
  return (
    <div className="self-stretch flex items-center justify-between gap-2 text-[9px] font-mono text-tagma-muted-dim">
      {attribution ? (
...
function ToolBody({ part, state }: { part: ToolPart; state: ToolState }) {
  ...
  const body = renderedBody ?? (
    <>
      ...
      {state.status === 'error' && (
        <pre className="select-text text-[9px] text-tagma-error/90 whitespace-pre-wrap break-all overflow-hidden">
          {state.error}
        </pre>
      )}
    </>
