...
function UserMessageContextNote({ entry }: { entry: OpencodeThreadEntry }) {
  ...
  return (
    <div className="text-tiny font-mono text-tagma-muted/60" title={tooltip} aria-label={tooltip}>
...
function AssistantResponseControls({ info, text }: { info: AssistantMessage; text: string }) {
  ...
  return (
    <div className="self-stretch flex items-center justify-between gap-2 text-tiny font-mono text-tagma-muted-dim">
      {attribution ? (
...
function ReasoningPartView({ text, streaming }: { text: string; streaming: boolean }) {
  ...
  return (
    <details
      ...
      className={`chat-disclosure w-full text-caption font-mono border-l-2 pl-2 ${
...
function ToolPartView({ part }: { part: ToolPart }) {
  ...
  return (
    <details
      className={`chat-disclosure text-caption font-mono w-full border bg-tagma-surface ${
        running
    ...
    >
      ...
      >
        ...
        <span className={`shrink-0 text-tiny ${statusTone}`}>{statusLabel}</span>
  ...
  const body = renderedBody ?? (
    <>
      {Object.keys(state.input).length > 0 && (
        <pre className="select-text text-tiny text-tagma-muted/80 whitespace-pre-wrap break-all overflow-hidden">
          {safeStringify(state.input)}
        </pre>
      )}
      {state.status === 'completed' && state.output && (
        <pre className="select-text text-tiny text-tagma-text/90 whitespace-pre-wrap break-all overflow-hidden max-h-[240px] overflow-y-auto">
          {state.output}
        </pre>
      )}
      {state.status === 'error' && (
        <pre className="select-text text-tiny text-tagma-error/90 whitespace-pre-wrap break-all overflow-hidden">
          {state.error}
        </pre>
      )}
    </>
