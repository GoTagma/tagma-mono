...
export function FileExplorer({
  ...
  const loadDir = useCallback(
    async (dirPath?: string) => {
      ...
      setAnchorPath(null);
      try {
        const result = await api.listDir(dirPath, { picker, capabilityPurpose });
    ...
    [mode, fileFilter, picker, capabilityPurpose],
  );
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {!loading &&
            ...
            entries.map((entry) => {
              const isSelected =
                multi && entry.type !== 'directory' && selected.includes(entry.path);
              return (
                <button
                  key={entry.path}
