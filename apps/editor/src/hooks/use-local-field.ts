...
let lastLocalFieldEditAt: number | null = null;

export function getLastLocalFieldEditAt(): number | null {
  return lastLocalFieldEditAt;
}

export function clearLastLocalFieldEditAt(): void {
  lastLocalFieldEditAt = null;
}
...
const activeDiscardFns = new Set<() => void>();
...
/**
 * Drop every pending local-field edit without committing it. Used when the
 * user explicitly chooses the disk/agent version during a conflict: stale
 * debounced field commits must not re-apply the discarded canvas afterwards.
 */
export function discardAllLocalFieldEdits(): void {
  clearLastLocalFieldEditAt();
  for (const discard of activeDiscardFns) {
    discard();
  }
}
...
export function useLocalField(
  ...
  const discardPendingLocalEdit = useCallback(() => {
    clearDebounce();
    const next = pendingServerRef.current ?? committedRef.current;
    setLocal(next);
    localRef.current = next;
  ...
  useEffect(() => {
    ...
    activeDiscardFns.add(discardPendingLocalEdit);
    return () => {
      activeFlushFns.delete(flushCommit);
      activeDiscardFns.delete(discardPendingLocalEdit);
  ...
  }, [discardPendingLocalEdit, flushCommit]);
