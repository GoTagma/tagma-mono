...
export function buildChatYamlTrialPlanPrompt(
  ...
  return [
    ...
    'Think through observable behavior before testing. Assemble the plan sequentially with bounded tagma_trial_plan operations: begin once, upsert-case once per case, set-coverage once, set-findings once, then commit exactly once. Never send the whole plan or multiple cases in one call. Pass the exact staged Target YAML path shown above, even if the OpenCode session directory points elsewhere.',
