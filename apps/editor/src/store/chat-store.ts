...
export function buildChatYamlTrialPlanPrompt(
  ...
  return [
    ...
    'Think through observable behavior before testing. Assemble the plan sequentially with bounded tagma_trial_plan operations: begin once, upsert-case once per case, set-coverage once, set-findings once, then commit exactly once. Begin resumes a matching path-and-hash draft by default; use reset true only to rebuild it from scratch. Never send the whole plan or multiple cases in one call. Pass the exact staged Target YAML path shown above, even if the OpenCode session directory points elsewhere.',
    'Never copy YAML or plan files between staging and live .tagma. Only commit consumes the formal attempt; it binds the plan to the current YAML hash and validates the complete plan before writing. Fix a reported pre-commit contract error by retrying only that bounded tool operation, not with filesystem copies.',
    ...
    'Blocked coverage is always diagnostic-only and never authorizes a later YAML repair. Accepted risk produces a visible passed-with-warnings result rather than a claim of full verification. Do not relabel a harness limitation as a pipeline defect, invent passing expectations, remove legitimate prerequisites, or weaken manual approvals/safety gates.',
    '',
