...
export function buildChatYamlTrialPlanPrompt(
  ...
  return [
    ...
    'Every finding needs repairScope: pipeline-artifact only for a concrete YAML or agent-owned companion defect; harness, environment, service, credential, approval, and unsupported-observation limits are diagnostic-only.',
    'Blocked coverage is diagnostic-only and cannot authorize YAML repair. accepted-risk is visible as passed-with-warnings. Never turn a harness limit into a pipeline defect, invent a pass, remove prerequisites, or weaken approvals or safety gates.',
    '',
