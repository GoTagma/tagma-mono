...
- The server Chat V2 diagnostics contributor may expose only a bounded tail of Host operation
  ...
  Trial events may additionally project only the validated `trialProgress` step enum, paired
  one-based case/run counters, and ordered start/semantic/heartbeat timestamps. Reuse the Host
  event contract, omit malformed/older evidence, and keep context and cursor pages identical.
  Never forward stage/trial ids, case titles, or free-text progress through this projection.
...
- Requirements verification guidance checks executable availability by platform, never assumes
  every CLI supports `--version`, and never executes `probe` during preflight. Keep legacy probe
  parsing and unlabelled `Verify:` rendering compatible. Recognize old generated bodies exactly
  before upgrading them; preserve customized bodies and platform-independent generated bytes.
  Both dependency-help views must show the same labelled verification instructions.
