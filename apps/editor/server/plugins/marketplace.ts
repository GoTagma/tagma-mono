...
export async function fetchWeeklyDownloads(name: string): Promise<number | null> {
  // The downloads cache stores `null` to mean "registry told us nothing
  // useful" — treat it as a real hit, not a miss. cacheGet collapses both
  // cases to `null`, so check the entry directly here.
  const entry = marketplaceDownloadsCache.get(name);
  if (entry) {
