...
export function assertWithinNodeModules(pluginDir: string, workDir: string): void {
  ...
  // Defense-in-depth: even if a future regex relaxation lets a name like
  // `@scope/..` slip through, `pluginDirFor()` resolves it to the
  // node_modules root itself. `isPathWithin(x, x)` returns true (root
  // is within itself per the shared contract), so the containment check
  // alone would pass and a downstream `rmSync(pluginDir)` would wipe
  // the entire node_modules tree. Reject the equality case explicitly.
  if (resolve(pluginDir) === nodeModulesRoot) {
    throw new PluginSafetyError(
      `Plugin directory cannot resolve to node_modules itself: ${pluginDir}`,
    );
  }
 ...
 *
 * @deprecated Use `tagmaPlugin` field in package.json (via `readPluginManifest`
 * from @tagma/sdk) as the canonical signal. This function will be removed in
