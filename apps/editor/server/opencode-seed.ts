...
Mark a dimension covered only when concrete linked case evidence exercises it. A fixed single-input surface is not-applicable for multiple-inputs and duplicate-input-names unless cases genuinely vary that surface. When a required dimension cannot be covered because the pipeline persists no deterministic artifact to observe it, that is a fixable verifiability gap: record a blocking finding with \`repairScope: pipeline-artifact\` naming the missing artifact. Mark a dimension \`blocked\` only when no pipeline repair can expose it to the harness.
...
Use file-equals when exact text preservation matters, including an empty expected string for empty-content cases. Use exact text or later-paragraph markers so a first-line-only implementation cannot pass. If the pipeline itself deterministically creates files that downstream tasks consume as inputs, leave them out of \`fixtures\`, list their paths in \`generatedInputPaths\`, and add an exact \`file-equals\` expectation for every listed path. The Host will not pre-seed those paths, rejects overlap with \`fixtures\`, and verifies the targeted closure really produced the expected bytes. Never add dummy fixtures at paths the pipeline deletes or recreates merely to satisfy coverage. If empty-content (or any other required dimension) cannot be asserted because the pipeline persists no deterministic artifact, record a blocking \`pipeline-artifact\` finding naming the missing artifact instead of marking it blocked.
