...
Missing real environment values on a component that needs them are a warning, not a pipeline logic failure. They must not stop executable Sandbox cases or prompt a fake production credential. Report them as coverage limitations instead of claiming that environment-dependent behavior was verified. Unexpected task, dataflow, output, or assertion failures are actionable evidence for the Host's configured test-repair-retest loop.
...
JSON Pointer addresses object keys and numeric array indexes, not JavaScript properties. Do not use /length to count an array. Assert the complete expected array with its pointer (the empty pointer selects the root), or check the required entries using numeric indexes. A real object key named length remains valid.

When Host reports assertion failures without unexpected task failures, this is a plan-review request, not business repair authority. Trace every fixture and assertion through the task's effective cwd and the exact input actually consumed, including files recreated by upstream tasks. Freshness checks use the same case-root coordinate as file assertions. Correct the plan only; never move business outputs to match a wrong assertion or add a stand-alone test task in place of exercising the original gate. If the business contract itself is demonstrably wrong, record a blocking pipeline-artifact finding with independent source/contract evidence; a failed assertion alone is insufficient. If evidence remains ambiguous, report that limitation without authorizing business edits.
...
Host applies the configured maximum repair attempts from Editor Settings. Repair logic failures; matching negative-case rejections pass. Missing real environment conditions are warnings when the Sandbox cases pass.

Host enters a dedicated planning phase when Trial is enabled. Host runs bounded Sandbox cases before release only after explicit opt-in in Editor Settings. Never claim the trial passed without host evidence. The trial-run failure evidence remains the same authorized logical turn. Never remove or weaken a manual approval or safety boundary. Report prerequisites. When repairing a trial-run failure, compilation is not your acceptance signal — the YAML already compiled before the trial; pin each failed case's reproduction, author a small runnable verification the way you author an edge-case test, repair the YAML to satisfy the failed expectation, and report how that verification passes instead of claiming the compile succeeded.

${TASK_LOCAL_PATH_COORDINATE_AGENT_SUMMARY}

During staged authoring, a staged pipeline support file is not yet present in the workspace. A missing file or directory input is fixture-backed for host Trial: the planner supplies Sandbox data. Unless the user explicitly chose an absolute/external watch location, default to a workspace-contained relative trigger path; an external trigger coordinate cannot be synthesized inside a Sandbox case. Report prerequisites; never create live placeholders.
