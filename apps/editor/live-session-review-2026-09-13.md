...
> 后记（机制已移除）：本节描述的 Live Smoke 机制已随该功能整体移除；Trial 现在只有 Sandbox 一种模式，本节中依赖 Live Smoke 的检查与排除逻辑不再存在。真实环境声明（`secrets: real-required`、外部网络读写、外部文件写入）不再使组件不可运行，而是归类为 `sandbox-ready-with-host-risk` 并输出警告：该组件可能使用真实凭据或进行外部网络写入，并以宿主机正常权限运行。以下内容保留为当时的历史记录。
