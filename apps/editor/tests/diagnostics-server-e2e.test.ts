...
test('real sidecar exposes only the temporary read-only diagnostics protocol', async () => {
  ...
  try {
    ...
    const manifestResponse = await fetch(`${enabled.connection.baseUrl}/manifest`, {
    ...
    const contextResponse = await fetch(`${enabled.connection.baseUrl}/context`, {
      headers: diagnosticsHeaders,
    });
