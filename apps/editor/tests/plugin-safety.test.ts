...
import { isValidPluginName, PLUGIN_NAME_RE } from '@tagma/sdk/plugins';
...
describe('isValidPluginName', () => {
  ...
  test.each([
    ...
    '', // empty
    // Single-segment dot escapes — these previously slipped past the regex
    // because `[a-z0-9._-]+` accepts `..`/`.`. `pluginDirFor` resolved them
    // to node_modules itself, letting an uninstall delete the whole tree.
    '@x/..',
    '@x/.',
    '@x/...',
    '@x/.._',
...
describe('assertWithinNodeModules', () => {
  ...
  test('rejects node_modules itself', () => {
    expect(() =>
      assertWithinNodeModules(resolve(FAKE_WORKDIR, 'node_modules'), FAKE_WORKDIR),
    ).toThrow(PluginSafetyError);
  });
...
describe('isPathWithin', () => {
  ...
  test('rejects a new child path when an existing parent is a symlink outside root', () => {
    const root = join(
      tmpdir(),
      `tagma-path-root-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    );
    const outside = join(
      tmpdir(),
      `tagma-path-outside-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    );
    try {
      ...
      expect(isPathWithin(join(root, 'link', 'new.yaml'), root)).toBe(false);
    } finally {
