...
Add capability plugin support while supporting old plugin modules.
...
- In-repo plugins load through the new capability shape.
- New multi-capability plugins load.
- Registry replacement warnings remain available.
...
- Show `createTagma` as the primary API.
- Show explicit subpath imports for YAML/config/plugin helpers.
- Explain runtime/plugin composition.
- Replace `ports` examples with unified `inputs` / `outputs`.
- Add migration guide.

Add docs:

- `docs/sdk-public-api.md`
- `docs/sdk-plugin-authoring.md`
- `docs/sdk-runtime-authoring.md`
- `docs/sdk-migration-v0-to-v1.md`
...
Old plugins may break if the registry changes too quickly. Mitigation: migrate plugin modules deliberately when Phase 3 introduces the capability model.

## 13. Phase Status

Phase 0 and Phase 1 are the current change set:

- [x] Add `createTagma`.
- [x] Add `Tagma` instance wrapper.
- [x] Remove root `runPipeline`; public usage goes through `createTagma().run(...)`.
- [x] Remove process-wide registry helpers and require explicit registry composition.
- [x] Add tests for isolated plugin registries and explicit registry requirements.
- [x] Update README quick start and in-repo imports to the new API.

Phase 2 starts after these checks pass and focuses only on splitting `engine.ts` internals without changing the new public API.
