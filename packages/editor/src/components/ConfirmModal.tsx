...
export function ConfirmModal({ info, onClose }: ConfirmModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[210] flex items-center justify-center bg-black/60" onClick={handleDismiss}>
      <div
      ...
      >
        <div className="panel-header">
          <div className="flex items-center gap-2 min-w-0">
            <AlertCircle size={14} className={`shrink-0 ${info.danger ? 'text-tagma-error' : 'text-tagma-accent'}`} />
            <h2 className={`panel-title truncate ${info.danger ? 'text-tagma-error' : 'text-tagma-text'}`}>
              {info.title}
            </h2>
          </div>
          <button onClick={handleDismiss} className="p-1 text-tagma-muted hover:text-tagma-text" aria-label="Close dialog">
            <XIcon size={14} />
          </button>
        </div>
        ...
        <div className="px-4 py-3 border-t border-tagma-border flex justify-end gap-2">
          ...
          <button
            ...
            className={info.danger ? 'btn-danger' : 'btn-primary'}
          >
            {info.confirmLabel}
          </button>
        </div>
