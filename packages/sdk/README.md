...
Input bindings support `value`, `from`, `default`, `required`, optional `type`, `enum`, and `description`. When an input has no `value` or `from`, Tagma first looks for a same-name output from the task's direct upstreams, then falls back to `default`. Use `from` when you need to rename, disambiguate, or read task streams. Common forms are `build.bundlePath`, `build.outputs.bundlePath`, `outputs.bundlePath`, `build.stdout`, and the fully-qualified `track.build.outputs.bundlePath` when the short task id would be ambiguous.

Every required input without a `value` or `default` must have exactly one producer among its direct dependencies; a transitive ancestor does not count. A required task-specific `from` without a fallback must name an output that dependency can produce. Same-name and `outputs.name` lookups are rejected when multiple direct dependencies can produce the name, even for optional inputs or bindings with a `default`, because a fallback cannot choose between producers.
...
| `from`        | `string`                                                | No                | Inputs: optional upstream source for rename/disambiguation or raw fields such as `taskId.name`, `taskId.outputs.name`, `outputs.name`, or `taskId.stdout`. Unset inputs auto-match same-name upstream outputs. Outputs: `json.<key>`, `stdout`, `stderr`, or `normalizedOutput` |
...
Set `lifecycle.repair: true` for a finite, success-conditioned repair loop. Repair requires a numeric `max_runs` of at least `2` and either omits `stop_when` or sets it to `success`; `max_runs: infinite`, `stop_when: failure`, and `stop_when: always` are rejected. After a failed non-final attempt, the next attempt receives bounded, redacted task status, exit-code, failure-kind, stdout, and stderr evidence in a `[Previous attempt failure]` block on every prompt task. The SDK also seeds each prompt task with its own previous session and driver when available, allowing native session resume; a bounded, redacted copy of normalized output is retained as fallback context. If a task declares `continue_from`, that current-attempt upstream handoff takes priority over same-task retry state.
